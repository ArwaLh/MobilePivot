...
export default class gestionNaevus extends Component {
	...
	componentDidMount(){
		AsyncStorage.getItem('medecin_patient').then((patient_medecin_arrayy) => {
			...
			this.itemsRef.child('medecins').child(arr.medecin_id).child('categories').child('grain_de_beaute').child('patients').child(arr.patient_id).child('dossiers_medicaux').on('value', (snap) => {
			let items=[];
			...
			let patient_a=null;
			this.itemsRef.child('medecins').child(arr.medecin_id).child("categories").child("grain_de_beaute").child('patients').orderByKey().equalTo(arr.patient_id).once("child_added", function(snapshot) {
...
/* 	localiser_photo(id){
	...
	nouveau_dossier(){
		...
		this.itemsRef.child('medecins').child(this.state.medecin_id).child("categories").child("grain_de_beaute").child('patients').child(this.state.patient_id).child('dossiers_medicaux').child(dossier_id).set({ 
